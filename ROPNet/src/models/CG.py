...
class PointNet(nn.Module):
    def __init__(self, in_dim, gn, out_dims, cls=False, dropout_prob = 0.2):
        ...
        for i, out_dim in enumerate(out_dims):
            ...
            # if dropout_prob > 0:#Added
            #     self.backbone.add_module(f'pointnet_dropout_{i}',
    ...
    def forward(self, x, pooling=True):
        ...
        #f = F.dropout(f, p=0.5, training=self.training)#Added
        g, _ = torch.max(f, dim=2)
        return f, g


class MLPs(nn.Module):
    def __init__(self, in_dim, mlps, dropout= 0.2):
        ...
        for i, out_dim in enumerate(mlps):
            ...
            if i != l - 1:
                ...
                self.mlps.add_module(f'dropout_{i}', nn.Dropout(p=dropout))
            in_dim = out_dim
...
class CGModule(nn.Module):
    def __init__(self, in_dim, gn):
        ...
        self.encoder = PointNet(in_dim=in_dim,
                                gn=gn,
                                out_dims=[64*3, 64*3, 64*3, 128*3, 512*3])
        self.decoder_ol = PointNet(in_dim=2048*3,
                                   gn=gn,
                                   out_dims=[512*3, 512*3, 256*3, 2],
                                   cls=True)
        self.decoder_qt = MLPs(in_dim=1024*3,
                               mlps=[512*3, 512*3, 256*3, 7])
