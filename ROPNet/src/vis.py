...
from data import RANSAC,RigidCPD
...
def vis_ROPNet(args, test_loader):
    ...
    with torch.no_grad():
        for i, (tgt_cloud, src_cloud, gtR, gtt) in tqdm(enumerate(test_loader)):
            ...
            src.append(np.array(src_cloud.points))
            tgt.append(np.array(tgt_cloud.points))
            ts_src.append(np.array(pred_cloud.points))
    data = {'source':src,'target':tgt,'pred':ts_src}
...
if __name__ == '__main__':
    ...
    test_set = RANSAC(root=args.root,
                          split='test',
                          npts=args.npts,
                          p_keep=args.p_keep,
                          noise=args.noise,
