...
from ngenet.data import ShapeDataset, EarDataset
from .MRI import MRIDataset
import ngenet.cpp_wrappers.cpp_neighbors.radius_neighbors as cpp_neighbors
import ngenet.cpp_wrappers.cpp_subsampling.grid_subsampling as cpp_subsampling
...
def collate_fn(list_data, config, neighborhood_limits):
    ...
    for item in list_data:
        ...
        transf = item['transf']
        coors = item['coors']
        #tgt_points_full = item['tgt_points_full']
        batched_points_list.append(src_points)
        batched_points_list.append(tgt_points)
        #batched_full_tgt.append(tgt_points_full)
        ...
        batched_coors_list.append(torch.from_numpy(coors).long())
        #batched_idx_list.append(torch.from_numpy(inds).long())
        #batched_faces_list.append(torch.from_numpy(faces).long())
    ...
    #batched_points_tgt_full = torch.from_numpy(np.concatenate(batched_full_tgt, axis=0).astype(np.float32))
    ...
    #batched_faces = batched_faces_list
    ...
    dict_inputs = {
        ...
        #'points_tgt_full': batched_points_tgt_full
    }
    return dict_inputs
...
def calibrate_neighbors(dataset, config, collate_fn, keep_ratio=0.8, samples_threshold=2000):
    ...
    for i in range(len(dataset)):
        ...
        dict_inputs = collate_fn(list_data=[dataset[i]], # FIXME ERROR IN DATASET
                                 config=config,
...
def get_dataset(dataset_name, config):
    assert dataset_name in ['shapedataset', 'eardataset', 'MRIDataset']

    if dataset_name == 'shapedataset':
        train_dataset = ShapeDataset(root=config.root,
                                    shape=config.shape,
                                    split='train',
                                    aug=True,
                                    overlap_radius=config.overlap_radius)
        val_dataset = ShapeDataset(root=config.root,
                                    shape=config.shape,
                                    split='val',
                                    aug=False,
                                    overlap_radius=config.overlap_radius)
    ...
    elif dataset_name == 'eardataset':
        train_dataset = EarDataset(root=config.root,
        ...
        return train_dataset, val_dataset
    elif dataset_name == 'MRIDataset':
        train_dataset = MRIDataset(root=config.root,
                                    split='train',
                                    aug=True,
                                    overlap_radius=config.overlap_radius)
