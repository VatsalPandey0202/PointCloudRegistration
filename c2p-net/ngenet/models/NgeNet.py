...
from ngenet.models.KPConv import block_decider
from ngenet.models import InformationInteractive
...
class NgeNetCoors(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.model = NgeNet(config)
        self.mlp = nn.Conv1d(32, 1, 1)
    ...
    def forward(self, inputs):
        ...
        coors = torch.sigmoid(self.mlp(feats_src_l.T.unsqueeze(0))).squeeze().T
        return coors