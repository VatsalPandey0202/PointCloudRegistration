...
from ngenet.data import get_dataset, get_dataloader
from ngenet.models import architectures, NgeNet
from ngenet.losses import Loss
from ngenet.utils import decode_config, setup_seed
...
def main():
    ...
    config = edict(config)
    if config.dataset in architectures.keys():
        config.architecture = architectures[config.dataset]
    else:
        config.architecture = architectures['shapedataset']
    ...
    for epoch in range(config.max_epoch):
        ...
        for inputs in tqdm(train_dataloader):
            ...
            coords_src = stack_points[:stack_lengths[0][0]]
            coords_tgt = stack_points[stack_lengths[0][0]:]
        ...
        with torch.no_grad():
            for inputs in tqdm(val_dataloader):
                ...
                # If GPU memory allows, it is recommended not to add this line of code or add this line after each epoch
                torch.cuda.empty_cache()
        print(f'Train loss: {float(t_loss/train_step)}, Val loss: {sum(total_loss)/val_step}',)
